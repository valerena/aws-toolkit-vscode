...
export class LoginManager {
    ...
    public async login(args: { passive: boolean; providerId: CredentialsId }): Promise<boolean> {
        ...
        try {
            ...
            const cachedCredentials = await this.store.upsertCredentials(args.providerId, provider)
            const credentials = cachedCredentials.credentials
            const endpointUrl = cachedCredentials.endpointUrl
            if (!credentials) {
    ...
    public async validateCredentials(credentials: Credentials, endpointUrl?: string, region = this.defaultCredentialsRegion) {
